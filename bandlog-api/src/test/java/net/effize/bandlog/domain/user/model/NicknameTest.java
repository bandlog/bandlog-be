...
public class NicknameTest {
    ...
    public void randomNickname은_null이_아닌_Nickname_객체를_반환해야_한다() {
        ...
        // act
        Nickname nickname = Nickname.randomNickname(random);
    ...
    public void randomNickname으로_생성된_닉네임은_정해진_패턴을_따라야_한다() {
        ...
        // assert
        assertThat(nicknameValue).matches("(소름돋는|기가막힌|천년에한번나올|경악스러운|온몸에전율이돋는|짜릿한|감동이흐르는|참을수없는)(재즈|펑크|메탈|락|팝|인디)(기타리스트|보컬리스트|드러머|베이시스트|키보디스트)");
    }

    @Test
    public void 동일한_Random_시드로_생성된_닉네임은_같아야_한다() {
        // arrange
        Random random1 = new Random(123);
        Random random2 = new Random(123);

        // act
        Nickname nickname1 = Nickname.randomNickname(random1);
        Nickname nickname2 = Nickname.randomNickname(random2);
    ...
    public void 여러_번_호출하면_다양한_닉네임이_생성된다() {
        ...
        String firstNickname = Nickname.randomNickname(random).stringValue();

        for (int i = 0; i < 10; i++) {
            String currentNickname = Nickname.randomNickname(random).stringValue();
